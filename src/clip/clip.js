...
class ImgClip {
    ...
    resizeCanvas(width, height) {
        ...
        const legalWidth = this.oldWidth;
    ...
    resizeClip() {
        ...
        const moving = (e) => {
            ...
            if (isConstrain) {
                curY = Math.min(curY, this.container.offsetHeight);
                curY = Math.max(0, curY);
                curX = Math.min(curX, this.container.offsetWidth);
            ...
            this.rectOldLeft = left;
            this.rectOldTop = top;
            this.rectOldWidth = width;
            this.rectOldHeight = height;
    ...
    draw() {
        // 放大镜
        this.drawMag();
        const realImgSize = this.getRealFinalImgSize
            (this.clipRect.offsetWidth * this.RATIO_PIXEL,
                this.clipRect.offsetHeight * this.RATIO_PIXEL);
        let curWidth = realImgSize.width;
        let curHeight = realImgSize.height;
        // clipTips,canvas之外的
    ...
    getClipRectParams() {
        let srcX = this.clipRect.offsetLeft;
        let srcY = this.clipRect.offsetTop;
        let sWidth = this.clipRect.offsetWidth;
        let sHeight = this.clipRect.offsetHeight;
        
        const offsetTop = this.clipRect.offsetTop;
        const offsetLeft = this.clipRect.offsetLeft;
